        ...
        <div class="rating">
          ...
          <div class="rating-wrapper">
            <ul v-show="food.ratings && food.ratings.length">
              <!--函数式定义v-show，这里绑定评论分给予其内容，显示出来，函数在method中-->
              <li v-show="needShow(rating.rateType,rating.text)" v-for="rating in food.ratings" class="rating-item border-1px" :key="rating.rateType">
                ...
                <!--使用filter来更改事件的格式,在这里添加|,在js中使用filters方法来定义-->
...
//      这里将评价内容与好评中评分级相关联，并给到标签的v-show以显示
      needShow(type, text) {
//        选中只有内容，且文本为空
...
//        必须使用dom异步更新bscroll才有效，使得更改评论选项时滚动正常
        this.$nextTick(() => {
          this.scroll.refresh();
        });
      }
    },
//    在这里更改时间格式,然后用到的formatDate(date, 'yyyy-MM-dd hh:mm')在src->common->js->date.js中
    filters: {
      formatDate(time) {
...
<style lang="scss" rel="stylesheet/scss">
  ...
  .food {
    ...
    .image-header {
      ...
      height: 0;
      padding-top: 100%;
      /*padding-top 100% 默认和width相等，就会生成一个长宽相等的正方形图片*/
      img {
        position: absolute;
        top: 0;
        left: 0;
